...
import (
	...
	legacyrouter "github.com/wallarm/api-firewall/internal/platform/routers/legacy"
)

func prepareJsonData(s string) *fastjson.Value {
	var parser fastjson.Parser
	value, _ := parser.Parse(s)
	return value
}
		...
		t.Run(tg.name, func(t *testing.T) {
			for _, tc := range tg.testCases {
				t.Run(tc.name, func(t *testing.T) {
					...
					httpReq.URI().SetHost("test.org")
					httpReq.SetRequestURI("test.org/test" + tc.path)
					...
					t.Log(httpReq.URI().String())
					t.Log(pathParams)
					t.Log(tc.param)

					input := &RequestValidationInput{RequestCtx: &reqCtx, PathParams: pathParams, Route: route}
					t.Log(len(input.GetQueryParams().PeekMulti("param")))
...
func TestDecodeBody(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			bodyRaw: []byte(urlencodedSpaceDelim.Encode()),
			schema: openapi3.NewObjectSchema().
				WithProperty("a", openapi3.NewStringSchema()).
				WithProperty("b", openapi3.NewIntegerSchema()).
				WithProperty("c", openapi3.NewArraySchema().WithItems(openapi3.NewStringSchema())),
		...
		},
		{
			...
			bodyRaw: []byte(urlencodedPipeDelim.Encode()),
			schema: openapi3.NewObjectSchema().
				WithProperty("a", openapi3.NewStringSchema()).
				WithProperty("b", openapi3.NewIntegerSchema()).
				WithProperty("c", openapi3.NewArraySchema().WithItems(openapi3.NewStringSchema())),
		...
		},
		{
			...
			want: map[string]interface{}{"a": "a1", "b": prepareJsonData("10"), "c": []interface{}{"c1", "c2"}, "d": prepareJsonData("{\"d1\":\"d1\"}"), "f": "foo", "g": "g1"},
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			//data, err := ioutil.ReadAll(tc.body)

			parser := parserPool.Get()

			got, err := decodeBody(tc.body, tc.bodyRaw, tc.mime, schemaRef, encFn, parser)
...
func TestRegisterAndUnregisterBodyDecoder(t *testing.T) {
	var (
		...
		decoder     = func(body io.Reader, bodyByte []byte, ct string, schema *openapi3.SchemaRef, encFn EncodingFn, parser *fastjson.Parser) (interface{}, error) {
			var vv []interface{}
			for _, v := range strings.Split(strconvUtils.B2S(bodyByte), ",") {
				vv = append(vv, v)
			}
			return vv, nil
		}
	...
	got, err := decodeBody(body, bodyRaw, contentType, schema, encFn, parser)
	...
	require.Truef(t, reflect.DeepEqual(got, nil), "got %v, want %v", got, "nil")
}
