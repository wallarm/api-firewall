...
func ValidateRequestBody(c context.Context, input *RequestValidationInput, requestBody *openapi3.RequestBody) error {
	...
	body := input.RequestCtx.Request.Body()
	reqBody := ioutil.NopCloser(bytes.NewReader(body))
	...
	inputMIME := strconv.B2S(input.RequestCtx.Request.Header.Peek(headerCT))
	...
	parser := input.ParserJson.Get()
	defer input.ParserJson.Put(parser)

	encFn := func(name string) *openapi3.Encoding { return contentType.Encoding[name] }
	value, err := decodeBody(reqBody, body, inputMIME, contentType.Schema, encFn, parser)
	if err != nil {
		return &RequestError{
			Input:       input,
			RequestBody: requestBody,
