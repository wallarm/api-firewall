...
func TestFilter(t *testing.T) {
	...
	expectWithDecoder := func(req ExampleRequest, resp ExampleResponse, decoder ContentParameterDecoder) error {
		...
		httpReqNet, err := http.NewRequest(req.Method, req.URL, marshalReader(req.Body))
		require.NoError(t, err)
		httpReqNet.Header.Set(headerCT, req.ContentType)

		//Find route
		route, pathParams, err := router.FindRoute(httpReqNet)
		require.NoError(t, err)

		httpReq := fasthttp.AcquireRequest()
		httpReq.SetRequestURI(req.URL)
		httpReq.Header.SetMethod(req.Method)
		httpReq.SetBodyStream(marshalReader(req.Body), -1)
		...
		responseValidationInput := &ResponseValidationInput{
			RequestValidationInput: requestValidationInput,
			Status:                 resp.Status,
			ResponseHeader:         &reqCtx.Response.Header,
		}
		if resp.Body != nil {
		...
		err = ValidateResponse(responseValidationInput)
		require.NoError(t, err)
		return err
	}
...
func TestValidateRequestBody(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			req.SetBodyStream(tc.data, -1)

			if tc.mime != "" {
				req.Header.SetContentType(tc.mime)
			}
			...
			inp := &RequestValidationInput{RequestCtx: &reqCtx, ParserJson: &parserPool}
...
func TestRootSecurityRequirementsAreUsedIfNotProvidedAtTheOperationLevel(t *testing.T) {
	...
	for _, path := range tc {
		...
		httpReqNet := httptest.NewRequest(http.MethodGet, path.name, emptyBody)
		route, _, err := router.FindRoute(httpReqNet)
		require.NoError(t, err)
		httpReq := fasthttp.AcquireRequest()
		httpReq.SetRequestURI(path.name)
		httpReq.Header.SetMethod("GET")
		httpReq.SetBodyStream(emptyBody, -1)
