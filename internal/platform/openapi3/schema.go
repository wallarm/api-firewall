...
func (schema *Schema) VisitJSON(value interface{}, opts ...SchemaValidationOption) error {
	...
	switch value.(type) {
	case *fastjson.Value:
		return schema.visitFastJSON(settings, value.(*fastjson.Value))
	}
	return schema.visitJSON(settings, value)
}

func (schema *Schema) visitFastJSON(settings *schemaValidationSettings, v *fastjson.Value) error {
	if schema.IsEmpty() {
		return nil
	}
	if err := schema.visitFastSetOperations(settings, v); err != nil {
		return err
	}
	...
	case fastjson.TypeObject:
		return schema.visitFastJSONObject(settings, v)
	case fastjson.TypeArray:
		return schema.visitFastJSONArray(settings, v)
	case fastjson.TypeString:
		return schema.visitJSONString(settings, string(v.GetStringBytes()))
	case fastjson.TypeNumber:
		number, err := v.Float64()
		if err != nil {
			return err
		}
		return schema.visitJSONNumber(settings, number)
	case fastjson.TypeTrue, fastjson.TypeFalse:
		boolean, err := v.Bool()
		if err != nil {
			return err
		}
		return schema.visitJSONBoolean(settings, boolean)
	default:
		return &SchemaError{
			Value:       v,
			Schema:      schema,
			SchemaField: "type",
			Reason:      fmt.Sprintf("unhandled value of type %T", v),
		}
	}
}
...
func (schema *Schema) visitFastSetOperations(settings *schemaValidationSettings, value *fastjson.Value) (err error) {
	if enum := schema.Enum; len(enum) != 0 {
		for _, v := range enum {
			if string(value.GetStringBytes()) == v {
				return
			}
		}
		if settings.failfast {
	...
	if ref := schema.Not; ref != nil {
		...
		var oldfailfast bool
		oldfailfast, settings.failfast = settings.failfast, true
		err := v.visitFastJSON(settings, value)
		settings.failfast = oldfailfast
		if err == nil {
			if settings.failfast {
				return errSchema
	...
	if v := schema.OneOf; len(v) > 0 {
		...
		for _, item := range v {
			...
			if err == nil {
				if schema.Discriminator != nil {
					pn := schema.Discriminator.PropertyName
					if discriminatorVal := value.Get(pn); discriminatorVal != nil {
						mapref, okcheck := schema.Discriminator.Mapping[string(discriminatorVal.GetStringBytes())]
						if okcheck && mapref == item.Ref {
							ok++
						}
					}
...
func (schema *Schema) visitFastJSONArray(settings *schemaValidationSettings, value *fastjson.Value) error {
	if schemaType := schema.Type; schemaType != "" && schemaType != "array" {
		return schema.expectedType(settings, "array")
	}

	varray, err := value.Array()
	if err != nil {
		return err
	}
	...
	//varray
	if v := schema.UniqueItems; v && !isSliceOfUniqueItemsFastJson(varray) {
		if settings.failfast {
			return errSchema
		}
		err := &SchemaError{
	...
	if itemSchemaRef := schema.Items; itemSchemaRef != nil {
		...
		for i, item := range varray {
			if err := itemSchema.visitFastJSON(settings, item); err != nil {
...
func (schema *Schema) visitFastJSONObject(settings *schemaValidationSettings, value *fastjson.Value) error {
	...
	var reqValidationErr error
	o.Visit(func(key []byte, v *fastjson.Value) {
		k := strconvutils.B2S(key)
		if properties != nil {
			propertyRef := properties[k]
			if propertyRef != nil {
				p := propertyRef.Value
				if p == nil {
					reqValidationErr = foundUnresolvedRef(propertyRef.Ref)
					return
				}
				if err := p.visitFastJSON(settings, v); err != nil {
					if settings.failfast {
						reqValidationErr = errSchema
						return
					}
					err = markSchemaErrorKey(err, k)
					if !settings.multiError {
						reqValidationErr = err
						return
					}
		...
		if additionalProperties != nil || allowed == nil || (allowed != nil && *allowed) {
			if additionalProperties != nil {
				if err := additionalProperties.visitFastJSON(settings, v); err != nil {
					if settings.failfast {
						reqValidationErr = errSchema
						return
					}
...
func isSliceOfUniqueItemsFastJson(xs []*fastjson.Value) bool {
	s := len(xs)
